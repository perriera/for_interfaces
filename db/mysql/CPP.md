...
- Further, (as an exercise) add new methods to `sample::Blueprint` one for each type of SQL operation. Get them working in the mold first, the refined the test() cases for it. Then add the new methods to the C++ class. Then take a shot at testing these classes as part of the `sample_console` complete with parameter passing. 
