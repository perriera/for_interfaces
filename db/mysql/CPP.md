...
> **Given** we need to access a MySQL database from C++ (Ubuntu style) **when** we install `MySQL Connector/C++`  **then** we can manage data in a MySQL database using our `injections.io(injections_cpp)`-based C++ program
