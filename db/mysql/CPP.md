...
Molds allow the developer to work out details of a given interface independant of the implementation. Once the finite details of the molded interface meet the primary objective(s) of the JIRA entry the working code is then promoted to the C++ class level. We call this process of copying working code from the mold to the dock (and testing it again) **docking**

 - see [How to do dock/molds with injections.io(C++17) based projects](https://github.com/perriera/for_interfaces/blob/main/injections/how-to/do-dock-molds/README.md)
...
#### Your *first* `docking` demonstration:
...
See [Debug the program with VSC?](https://github.com/perriera/for_interfaces/blob/main/db/mysql/CPP.md#alternate-case-1) *optional* 
...
- Remember, whatever you do we never use that `word` any where in `injections.io(C++17)` source code, (C++ source code that is). At present you'll find our `bash` scripts are *still* using that `word` but we have figured out a way to *deprecate* it's use there too!
