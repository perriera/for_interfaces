...
> **Given** we wish build software using C++11/17 based tools for the Ubuntu platform **when** we update Ubuntu and install the `GNU Compiler Collection` and other build tools for Linux **then** we can build projects using g++ with support from automake, git, clangd cppcheck, checkinstall, gdb, ssh, net-tools, zip, xterm, curl plus others
